...
def getMoneyFlow(type: int, code: str, token: str = "", method: str = "get") -> str:
    """
    资金流向-实时接口
    获取沪深京股票交易日盘中资金流数据，盘中实时更新。
    
    :param type: 标的类型 - 1-沪深京A股, 4-ETF基金
    :param code: 股票代码。支持批量获取，最大50个。注意：不支持all参数获取。
    :param token: 登录网站获取token
    :param method: 请求方法，默认get
    :return: JSON字符串
    """
    if not token:
        token = Config.TOKEN
    url = Config.SERVER_URL + "/doc/hot/timemoney"
    params = {"type": type, "code": code, "token": token}
    return XTickUtil.request(url, method, params)


def getMoneyFlowHistory(type: int, code: str, startDate: str, endDate: str, token: str = "", method: str = "get") -> str:
    """
    资金流向-历史接口
    获取沪深京股票交易日资金流历史数据，盘后更新。
    ...
    if not token:
        token = Config.TOKEN
    url = Config.SERVER_URL + "/doc/hot/historymoney"
