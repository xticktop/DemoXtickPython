...
from xtick.scripts.api.XTickWatchApi import getDayKlineRealtime, getMinuteKlineRealtime, getDeepRealtime, getDeepHistory, getAmountStat, getFqKlineUpdate
...
from xtick.scripts.api.XTickHotApi import getLianbanTianti, getMarketEmotion, getMoneyFlow, getMoneyFlowHistory, getBidHistory, getBidDetail, getNews, getTimeKline, getConceptStocks, getStockConcepts, getDayUpdate
...
class XTickStockApiClient(object):
    ...
    def demoForWatchApi(self):
        ...
        # 3. 深度行情-实时
        print("\n[3] 深度行情-实时 - 获取平安银行实时深度行情:")
        result = getDeepRealtime(type=type_val, code=code, token=self.token)
        df = pd.DataFrame(json.loads(result))
        print(f"返回数据条数: {len(df)}")
        if len(df) > 0:
            print(df)
        ...
        # 4. 深度行情-历史
        print("\n[4] 深度行情-历史 - 获取平安银行昨日深度行情:")
        result = getDeepHistory(type=type_val, code=code, tradeDate=yesterday, token=self.token)
        data = json.loads(result)
        if data:
            df = pd.DataFrame(data)
            print(f"返回数据条数: {len(df)}")
        ...
        # 6. 日K线复权-更新
        print("\n[6] 日K线复权-更新 - 获取昨日全市场不复权日线:")
        result = getFqKlineUpdate(type=type_val, fq="1", tradeDate=yesterday, token=self.token)
        data = json.loads(result)
        if data:
            df = pd.DataFrame(data)
            print(f"返回数据条数: {len(df)}")
    ...
    def demoForHotApi(self):
        ...
        # 3. 资金流向-实时
        print("\n[3] 资金流向-实时 - 获取平安银行实时资金流向:")
        result = getMoneyFlow(type=type_val, code=code, token=self.token)
        data = json.loads(result)
        if data:
            df = pd.DataFrame(data)
            print(f"返回数据条数: {len(df)}")
        ...
        # 4. 资金流向-历史
        print("\n[4] 资金流向-历史 - 获取平安银行资金流向历史:")
        result = getMoneyFlowHistory(type=type_val, code=code, startDate=last_month, endDate=today, token=self.token)
        data = json.loads(result)
        if data:
            df = pd.DataFrame(data)
            print(f"返回数据条数: {len(df)}")
